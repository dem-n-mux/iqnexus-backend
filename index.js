...
app.get('/all-kindergarten-students', async (req, res) => {
  ...
  try {
    // Fetch all students with class KD
    const students = await KINDERGARTEN_STUDENT.find({ class: 'KD' })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();
...
app.post('/all-kindergarten-students-no-pagination', async (req, res) => {
  ...
  if (section && section.some(s => !['LK', 'UK', 'PG'].includes(s))) {
    return res.status(400).json({ message: 'Sections must be LK, UK, or PG' });
  }

  try {
    // Build query
    const query = { class: 'KD' }; // Enforce class as KG
...
app.delete('/kindergarten-student', async (req, res) => {
  ...
  if (!className || className !== 'KD') {
    return res.status(400).json({ message: 'Class must be KD' });
  }
